...
export const useItemAnimation = (speedValue, setSpeedPaused, currentLevel) => {
  ...
  const animateItem = () => {
    ...
    // Вычисляем длительность полёта на основе скорости (как у шара)
    ...
    gsap.set(itemRef.current, { x: '-2000px', opacity: 1 });
  ...
  return { itemRef, itemAnimating, animateItem, reverseItem, itemPosition };
};
